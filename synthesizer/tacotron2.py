...
class Tacotron2:
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, embed_filenames):
        ...
        for i in range(self._hparams.tacotron_num_gpus):
            ...
            input_seqs = np.concatenate((input_seqs, device_input), axis=1) if \
                input_seqs is not None else device_input
        ...
        for i, mel in enumerate(mels):
            ...
            # Note: outputs mel-spectrogram files and target ones have same names,
            ...
            if log_dir is not None:
                # save wav (mel -> wav)
                wav = audio.Spectrogram.inverse_spectrogram(mel.T, hparams, 'mel')
                audio.FileManagement.save_wav(wav,
                ...
                plot.plot_alignment(alignments[i],
                                    os.path.join(log_dir,
                                                 "plots/alignment-{}.png".format(basenames[i])),
                ...
                if hparams.predict_linear:
                    # save wav (linear -> wav)
                    wav = audio.Spectrogram.inverse_spectrogram(linears[i].T, hparams, 'linear')
                    audio.FileManagement.save_wav(wav,
                                                  os.path.join(log_dir,
    ...
    def _get_output_lengths(stop_tokens):
        # Determine each mel length by the stop token predictions.
        # (len = first occurence of 1 in stop_tokens row wise)
